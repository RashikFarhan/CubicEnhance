...
import { adminUpdate, cn, formatShortDate } from '@/lib/utils';
...
export default function ReviewsTable({ initialData }: { initialData: any[] }) {
  ...
  const stars = (n: number) => '⭐'.repeat(Math.min(5, Math.round(n || 0)));

  async function togglePublished(id: string, current: boolean) {
    try {
      await adminUpdate('reviews', id, { is_published: !current });
  ...
  async function handleSaveEdit(e: React.FormEvent) {
    ...
    try {
      const data = {
        ...
        rating: parseFloat(editingReview.rating) || 5,
      };
      await adminUpdate('reviews', editingReview.id, data);
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.map(r => (
              <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm">{stars(r.rating)} ({r.rating})</td>
                <td className="px-4 py-3 text-gray-600 text-xs">
                  <span className="font-semibold">{r.country}</span> · {r.service}
                </td>
                <td className="px-4 py-3 text-gray-500 text-xs">
                  {typeof r.created_at === 'string' ? new Date(r.created_at).toLocaleDateString() : '—'}
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => { setEditingReview(r); setError(''); }}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              ...
              <button onClick={() => setEditingReview(null)} className="text-2xl text-gray-400 hover:text-gray-600 leading-none">&times;</button>
            </div>
            <form onSubmit={handleSaveEdit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Review Text</label>
                <textarea value={editingReview.text || ''} onChange={e => setEditingReview({...editingReview, text: e.target.value})} className="w-full border p-2 rounded-lg text-sm" rows={4} required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Country</label>
                  <input type="text" value={editingReview.country || ''} onChange={e => setEditingReview({...editingReview, country: e.target.value})} className="w-full border p-2 rounded-lg text-sm" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Service</label>
                  <input type="text" value={editingReview.service || ''} onChange={e => setEditingReview({...editingReview, service: e.target.value})} className="w-full border p-2 rounded-lg text-sm" required />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Rating (1–5)</label>
                <input type="number" step="0.5" min="1" max="5" value={editingReview.rating || 5} onChange={e => setEditingReview({...editingReview, rating: e.target.value})} className="w-full border p-2 rounded-lg text-sm" required />
              </div>
